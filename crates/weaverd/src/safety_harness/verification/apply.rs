...
/// Returns the content length of a line, excluding trailing newline characters.
fn line_content_length(content: &str, line_start: usize, line_end: usize) -> usize {
    let adjusted_end = if line_end > 0 && content.as_bytes().get(line_end - 1) == Some(&b'\n') {
    ...
    adjusted_end.saturating_sub(line_start)
}
...
fn line_column_to_offset(
    ...
    let line_len = line_content_length(content, line_start, line_end);
...
mod tests {
    ...
    #[case::delete(
        "hello beautiful world",
        vec![TextEdit::delete_range(Position::new(0, 6), Position::new(0, 16))],
        "hello world"
    )]
    #[case::replace(
        "fn foo() {}",
        vec![TextEdit::from_positions(Position::new(0, 3), Position::new(0, 6), "bar".to_string())],
        "fn bar() {}"
    )]
    #[case::multiple_edits(
        "aaa bbb ccc",
        vec![
            TextEdit::from_positions(Position::new(0, 0), Position::new(0, 3), "AAA".to_string()),
            TextEdit::from_positions(Position::new(0, 8), Position::new(0, 11), "CCC".to_string()),
        ],
        "AAA bbb CCC"
    )]
    #[case::crlf_line_endings(
        "line one\r\nline two\r\nline three",
        vec![TextEdit::from_positions(Position::new(1, 5), Position::new(1, 8), "TWO".to_string())],
        "line one\r\nline TWO\r\nline three"
    )]
    #[case::lf_multiline(
        "first\nsecond\nthird",
        vec![TextEdit::from_positions(Position::new(1, 0), Position::new(1, 6), "SECOND".to_string())],
        "first\nSECOND\nthird"
    )]
    #[case::end_of_file_position(
        "hello",
        vec![TextEdit::insert_at(Position::new(0, 5), " world")],
        "hello world"
    )]
    fn apply_edits_succeeds(
        #[case] original: &str,
