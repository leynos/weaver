...
fn no_item_level_dead_code_allow() {
    assert!(
        ...
        "refactor_helpers.rs contains a forbidden item-level `#[allow(dead_code…)]`. Use \
         `#[expect(dead_code, reason = \"…\")]` if suppression is genuinely required, but prefer \
         restructuring so that it is not."
    );
}
...
fn no_anonymous_const_witness() {
    assert!(
        ...
        "refactor_helpers.rs contains an anonymous const dead-code witness (`const _: …`). These \
         witnesses were removed in issue `#89` and must not be re-introduced."
    );
}
