...
fn assert_three_part_output(
    ...
    let Some(error_pos) = output.find(error_text) else {
        anyhow::bail!("error text should be present: {error_text:?}\noutput:\n{output}");
    };
    let Some(alt_pos) = output.find(alternatives_text) else {
        anyhow::bail!(
            "alternatives text should be present: {alternatives_text:?}\noutput:\n{output}"
        );
    };
    let Some(next_pos) = output.find("Next command:") else {
        anyhow::bail!("next command header should be present\noutput:\n{output}");
    };
    ensure!(
...
fn write_actionable_guidance_produces_three_part_template() {
    ...
    .expect("actionable guidance must follow the three-part template");
}
...
fn write_bare_invocation_guidance_includes_all_domains() {
    ...
    assert_three_part_output(&output, "error:", "Usage:", "weaver --help")
        .expect("bare invocation guidance must follow the three-part template");
}
...
fn launch_daemon_guidance_uses_configured_binary_name() {
    ...
    .expect("launch guidance must follow the three-part template");
    assert!(
...
fn startup_guidance_surfaces_problem_and_next_command(
    ...
    assert_startup_guidance_template(&error, &expected)
        .expect("startup guidance must follow the expected template");
}
...
fn fallback_guidance_strips_existing_error_prefix() {
    ...
    .expect("fallback guidance must follow the three-part template");
}
