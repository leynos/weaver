...
fn fluent_localizer() -> Result<FluentLocalizer> {
    Ok(FluentLocalizer::with_en_us_defaults([WEAVER_EN_US])?)
}

fn assert_single_error_prefix(output: &str) -> Result<()> {
    ensure!(
...
fn assert_three_part_guidance(
    ...
    let Some(error_pos) = output.find(error_text) else {
        anyhow::bail!("expected error text: {error_text:?}\noutput:\n{output}");
    };
    let Some(alternatives_pos) = output.find(alternatives_text) else {
        anyhow::bail!("expected alternatives text: {alternatives_text:?}\noutput:\n{output}");
    };
    let next_command_line = format!("Next command:\n  {next_command}");
    let Some(next_command_pos) = output.find(&next_command_line) else {
        anyhow::bail!(
            "expected exact Next command block: {next_command_line:?}\noutput:\n{output}"
        );
    };
...
fn fluent_missing_operation_guidance_has_single_error_prefix() {
    ...
    .expect("missing-operation guidance must have three parts");
}
...
fn fluent_unknown_domain_guidance_has_single_error_prefix() {
    ...
    .expect("unknown-domain guidance must have three parts");
}
...
fn fluent_unknown_domain_guidance_uses_unique_suggestion_when_available() {
    ...
    .expect("suggested-domain guidance must have three parts");
}
