...
impl CacheWorld {
    fn request(&self) -> Result<CardExtractionInput<'_>> {
        Ok(CardExtractionInput {
            path: self.path.as_deref().context("path should be set")?,
            source: self.source.as_deref().context("source should be set")?,
            line: self.line,
            column: self.column,
    ...
    fn extractor(&self) -> Result<&TreeSitterCardExtractor> {
        self.extractor.as_ref().context("extractor should be set")
    }
}
...
fn given_second_fixture_extracted(world: &mut CacheWorld, name: String) -> Result<()> {
    let fixture_name = name.trim_matches('"');
...
fn when_extracted_twice(world: &mut CacheWorld) -> Result<()> {
    when_extracted_once(world)?;
    when_extracted_once(world)
}

#[when("the fixture is extracted once")]
fn when_extracted_once(world: &mut CacheWorld) -> Result<()> {
...
fn when_extraction_fails(world: &mut CacheWorld) -> Result<()> {
    when_extracted_once(world)?;
    ensure!(world.error.is_some(), "expected extraction to fail");
    Ok(())
}

#[then("the cache records {hits} hit")]
#[then("the cache records {hits} hits")]
fn then_cache_hits(world: &mut CacheWorld, hits: u64) -> Result<()> {
    ensure!(
        world.extractor()?.cache_stats().hits == hits,
...
fn then_cache_misses(world: &mut CacheWorld, misses: u64) -> Result<()> {
    ensure!(
        world.extractor()?.cache_stats().misses == misses,
...
fn then_timestamp_preserved(world: &mut CacheWorld) -> Result<()> {
    ensure!(world.cards.len() == 2, "expected two extracted cards");
    let first_timestamp = world
        .cards
        .first()
        .map(|card| card.provenance.extracted_at.as_str())
        .context("first cached card should exist")?;
    let second_timestamp = world
        .cards
        .get(1)
        .map(|card| card.provenance.extracted_at.as_str())
        .context("second cached card should exist")?;
    ensure!(
...
fn then_cache_entries(world: &mut CacheWorld, entries: usize) -> Result<()> {
    ensure!(
        world.extractor()?.cache_len() == entries,
