...
fn content_change_invalidates_cache() {
    ...
    let first =
        extract_card(&extractor, &rust_source("greet")).expect("card extraction should succeed");
    let second =
        extract_card(&extractor, &rust_source("welcome")).expect("card extraction should succeed");
...
fn cache_preserves_extraction_timestamp() {
    ...
    let first = extract_card(&extractor, &source).expect("card extraction should succeed");
    let second = extract_card(&extractor, &source).expect("card extraction should succeed");
...
fn cache_correctness_after_invalidation() {
    ...
    let original =
        extract_card(&extractor, &original_source).expect("card extraction should succeed");
    extractor.invalidate_path(rust_path());
    let updated =
        extract_card(&extractor, &updated_source).expect("card extraction should succeed");
...
fn extractor_reuses_parser_and_cache_for_identical_requests() {
    ...
    let parser_before = extractor
        .parser_identity(SupportedLanguage::Rust)
        .expect("parser identity should resolve");
    let _ = extract_card(&extractor, &source).expect("card extraction should succeed");
