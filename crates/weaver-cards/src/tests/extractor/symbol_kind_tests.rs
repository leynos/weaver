...
fn extracts_supported_symbol_kinds(#[case] case: SymbolExpectation<'static>) {
    let card = extract(case.request).expect("supported symbol extraction should succeed");
...
fn returns_module_cards_for_import_interstitials(#[case] request: ExtractRequest<'static>) {
    let card = extract(request).expect("module card extraction should succeed");
...
fn nested_locals_do_not_become_entities(#[case] request: ExtractRequest<'static>) {
    let card = extract(request).expect("nested-local extraction should succeed");
...
fn whitespace_only_edits_do_not_change_symbol_id() {
    ...
    .expect("compact Python card extraction should succeed");
    ...
    .expect("spaced Python card extraction should succeed");
