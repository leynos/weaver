...
pub fn render_matrix(manifest: &BoundaryManifest) -> String {
    let mut rendered = String::from(concat!(
        ...
        "`docs/orthoconfig-consumer-boundary.toml`. Do not edit the table\n",
    ...
    for phase in grouped_phases(manifest) {
        push_phase(&mut rendered, phase, manifest);
    }
...
fn grouped_phases(manifest: &BoundaryManifest) -> Vec<&str> {
...
fn push_phase(rendered: &mut String, phase: &str, manifest: &BoundaryManifest) {
    let rows = manifest
        .tasks
        .iter()
        .filter(|task| task.id.starts_with(phase))
        .map(MatrixRow::from)
        .collect::<Vec<_>>();
...
fn upstream_tasks(task: &BoundaryTask) -> String {
    let upstream = task
        .upstream
        .iter()
        .map(|reference| reference.task.as_str())
        .collect::<Vec<_>>()
        .join(", ");
