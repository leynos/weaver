...
def test_disposable_ubuntu_probe_excludes_fork_pull_requests() -> None:
    """The temporary diagnostic runs only for its same-repository branch."""
    guard = jobs()[("pr297-ubuntu-userns-diagnostic.yml", "sandbox-probe")].get("if")
    assert guard == (
        "github.event.pull_request.head.repo.full_name == github.repository && "
        "github.head_ref == 'diagnose-pr297-ubuntu-userns-20260924'"
    )
