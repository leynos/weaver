...
def test_disposable_coverage_probe_excludes_fork_pull_requests() -> None:
    """The temporary diagnostic cannot request its runner from a fork."""
    diagnostic = jobs()[
        ("pr297-ubuntu2204-coverage-diagnostic.yml", "coverage-probe")
    ]
    assert diagnostic.get("if") == (
        "github.event.pull_request.head.repo.full_name == github.repository && "
        "github.head_ref == 'diagnose-pr297-ubuntu2204-coverage-20260924'"
    )
