...
weaver's own workflows declare every runner as a scalar, so a reading that
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("${{ inputs.runner }}", set(), id="caller-input"),
    ],
)
...
def test_every_runner_declaration_is_readable(
    ...
    ``labels`` mapping, and every expression among them names its labels.
    A form the reader cannot see would otherwise read as "no runner", which
    exempts the lane from every placement, ceiling and registry assertion
    here at once; this names the coordinate instead.
    """
    definition = jobs()[coordinate]
    try:
        job_labels(definition)
    except RunnerShapeError as error:
        pytest.fail(f"{coordinate[0]}:{coordinate[1]}: {error}")


@pytest.fixture
def uncached_reader() -> Iterator[None]:
    """Clear the reader's parse caches before and after a redirected read.

    The reader caches the tree and the registry for the session, so a test
    pointing it at another directory must not see the repository's cached
    parse, and must not leave its own behind for the tests after it.
    """
    runner_placement_reader._parsed_workflows.cache_clear()
    runner_placement_reader._registered_labels.cache_clear()
    yield
    runner_placement_reader._parsed_workflows.cache_clear()
    runner_placement_reader._registered_labels.cache_clear()


@pytest.mark.usefixtures("uncached_reader")
