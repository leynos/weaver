...
- Enforce en-GB-oxendict (Oxford) spelling in Markdown with `make typos`. The
  `typos.toml` configuration is generated by
  `scripts/generate_typos_config.py`; regenerate it with
  `uv run scripts/generate_typos_config.py` rather than editing entries by
  hand. Add new curated word stems or accepted words to that script, and use its
  `extend-ignore-re` list for proper nouns and preserved template headings.
