...
For instance, if multiple tests require a logged-in user object or a
...
a tuple or struct and the test only cares about some parts or needs to use more
...
- `#[default(…)]`: Used within a fixture function's signature to provide default
  values for its arguments.
- `#[with(…)]`: Applied to a fixture argument in a test (or in another fixture)
  to supply explicit values and override any defaults.
...
a panic. For types with complex parsing logic or many failure modes, it might be
clearer to perform the conversion explicitly within a fixture or at the
...
`#[async_std::test]` to annotate the generated async test functions. However, it
is designed to be largely runtime-agnostic and can be integrated with other
...
of specific files or folders, or the availability of network services. It can be
used with `rstest`. For example, an `rstest` test could be further annotated
